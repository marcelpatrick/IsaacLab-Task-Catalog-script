...
- The problem this script fixes:
  ...
  * I generated an external project using the wizard project creator and, during its creation, I selected the specific libraries it should be created with.
  * Now I want to run different tasks in my project.
  * However, my external project only supports tasks that run on the libraries that it was generated with: (rl_games, rsl, skrl, ab3)
  * There are several tasks in the IsaacLab NVIDIA project. I want to be able to import and run multiple tasks from the original IsaacLab project into my external project but I don't know which tasks from the original project are supported by the libraries in my external project.
